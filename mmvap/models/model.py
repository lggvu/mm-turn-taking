...
from mmvap.models.modules import *
...
class GPTStereo(GPT):
    ...
    def forward(
        ...
        cross_attn_a = []
        cross_attn_b = []
        # print(f'in GPTStereo forward: x1 shape {x1.shape}, x2 shape {x2.shape}')
...
class StereoTransformerModel(VisualisationMixin, nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # encoder - flexible initialization based on config
        encoder_cfg = cfg['encoder_cfg']
        encoder_class_name = encoder_cfg.get('class', 'StereoEncoder')
        encoder_params = encoder_cfg.get('params', {})
        ...
        if encoder_class_name not in encoder_classes:
            raise ValueError(
                f"Unknown encoder class: {encoder_class_name}. "
                f"Available: {list(encoder_classes.keys())}"
            )

        encoder_class = encoder_classes[encoder_class_name]
        self.audio_encoder = encoder_class(**encoder_params).to("cuda")
        print(f"Initialized audio encoder: {encoder_class_name} with params: {encoder_params}")
    ...
    def forward(self, batch):
        ...
        if self.target_time_range is not None:
            print(f'chunk {self.chunk_count}: [{chunk_start_time:.2f}s - {chunk_start_time + self.window_size_sec:.2f}s]')
        self.chunk_count += 1

        if 'video_l' in batch:
            encoder_kwargs = dict(
                video_l=batch['video_l'].to("cuda"),
                video_r=batch['video_r'].to("cuda"),
                padding_mask=batch.get('video_padding_mask', None),
            )
            if 'audio_feats_l' in batch:
                # Precomputed log-fbank path (see run_inference_candor2 /
                # ValidationAudioVisualDataset2) -- skips the encoder's own raw-waveform
                # featurization entirely.
                encoder_kwargs['audio_feats_l'] = batch['audio_feats_l'].to("cuda")
                encoder_kwargs['audio_feats_r'] = batch['audio_feats_r'].to("cuda")
                audio = self.audio_encoder(**encoder_kwargs)
            else:
                audio = self.audio_encoder(batch['audio_chunk'].to("cuda"), **encoder_kwargs)
        else:
        ...
        out = self.cross_attention_transformer(x_l["x"], x_r["x"], return_attention=True)

        self._maybe_accumulate_attention(out['cross_attn'], chunk_start_time)
