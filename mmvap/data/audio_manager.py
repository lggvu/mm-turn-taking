...
def get_audio_info(audio_path):
    # print(f'getting audio info for {audio_path}')
...
class AudioManager():
    ...
    def load_waveform(
        ...
        if start_time is not None:
            ...
            x, sr = torchaudio.load(path, frame_offset=frame_offset, num_frames=num_frames, backend='sox', channels_first=channel_first)
        else:
            x, sr = torchaudio.load(path, backend='sox', channels_first=channel_first)

        if sample_rate: # before or after norm?
            resampler = torchaudio.transforms.Resample(orig_freq=info['sample_rate'], new_freq=sample_rate)
            if info['sample_rate'] != sample_rate:
                # print(f'resampling from {info["sample_rate"]} to {sample_rate}')
                x = resampler(x)
                sr = sample_rate
